...
| 序列化     | `serde` + `serde_json` | 1.x          | WAL 序列化                     | ✅ 复用        |
...
│   ├── wal.rs              # WAL 同步版（Phase 2 教学对照，未接入 Engine）
│   ├── wal_tokio.rs        # Write-Ahead Log 异步版（tokio mpsc + tokio::fs，生产路径）
...
| `wal.rs`                | WAL 同步版（教学对照） | std::thread、mpsc、Drop 优雅关闭      |
| `wal_tokio.rs`          | WAL 异步版（生产路径） | tokio::spawn、异步 channel、tokio::fs |
...
engine.rs: 加锁 → 操作 HashMap → 解锁
    │
    ├── PUT/DEL → wal_tokio.rs: 发送到 channel → 后台任务写文件
    ├── PUT --ttl → ttl.rs: 后台定时清理过期键
...
- **两个 `is_expired` 方法**：`is_expired()` 用 `Instant::now()` 实时判断（GET 懒检查与 TTL 批量清理均使用）；`is_expired_at(now)` 接收外部时间参数，预留给「同一批次需统一时间基准」的批量清理场景（当前未被使用）
- **不可序列化**：`Instant` 没有 `Serialize`/`Deserialize` 实现，`Entry` 不能直接序列化。持久化以 WAL 为准：TTL 以相对时长 `ttl_secs` 记录，恢复时用 `Instant::now() + ttl` 重算 `expires_at`（快照功能已取消，见 PRD §3.3 变更记录）
...
### 4.5 Engine 线程安全版（Phase 2 → 3 演进）

**学习目标：** Arc、Mutex、RAII 锁释放、Send/Sync、async/await

```rust
// Phase 2→3：RefCell→Mutex、Rc→Arc，WAL 迁移至异步版（与 src/engine.rs 一致）
...
pub struct Engine {
    ...
    config: Arc<Config>,
    wal: Option<AsyncWal>,  // 写前日志：先记 WAL 再改内存（Write-Ahead Log 语义）
    ttl: TtlManager,        // TTL 后台清理任务（见 §4.7）
}
...
impl Engine {
    ...
    pub async fn del(&self, key: &str) -> Result<bool, KvError> {
        if let Some(w) = &self.wal {
            w.append(WalOp::Del { key: key.to_string() }).await?;
        }
        let mut store = self.store.lock()?;
    ...
    /// 批量并发写入（Phase 3 async 版，与 src/engine.rs 实际实现一致）：
    /// WAL 在调用方 task 内串行 append（保证日志全序），
    ...
    pub async fn concurrent_put(
        ...
        for (key, value) in entries {
            if let Some(wal) = &engine.wal {
                ...
                .await?; // 日志失败则整批失败：WAL 不变量「内存不能领先于日志」
            }
            let store = Arc::clone(&engine.store); // Arc::clone 增加引用计数
            let handle = tokio::task::spawn(async move {
                let mut s = store.lock().unwrap(); // TODO：按「生产零 unwrap」规范整改
                s.insert(key, Entry::new(value, None));
            });
            handles.push(handle);
            // store 的 Arc 引用计数在 task 结束时 -1
        }
        Ok(handles)
    }
}
...
- **批量写入与 WAL**：`concurrent_put` 的 WAL 追加在调用方 task 内串行 `append().await`（而非 spawn 的并发 task 内）——单生产者保证日志全序（replay 顺序确定），且 `?` 能直接向调用方传播；放进并发 task 内则错误无法传播，只能 `let _ =` 吞掉，违背 WAL「内存不能领先于日志」的不变量
- **短路语义**：for 循环 + `?`：任一条 WAL 追加失败立即返回 Err，后续条目不再 append/spawn；已 spawn 的 `JoinHandle` 被丢弃（task detach），但已启动的 task 仍会完成插入，内存与 WAL 最终一致。同步版曾用 `Iterator::collect::<Result<_>>` 实现同样语义（collect 到 Result 遇第一个 Err 短路，闭包执行次数随之截断）
...
| `wal.rs` / `wal_tokio.rs` | 写入、channel 关闭、优雅关闭（同步 Drop / 异步 close） | 多线程/异步测试、drop-order 死锁陷阱 |
